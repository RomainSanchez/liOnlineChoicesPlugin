...
class ApiManifestationsService extends ApiEntityService
{
     ...
     * @return array
     */
    public function findAll(array $query, $includeEvents = true)
    {
        $q = $this->buildQuery($query);
        $manifestations = $q->execute();

        return $this->getFormattedEntities($manifestations, ['includeEvents' => $includeEvents]);
    }
    ...
    public function buildInitialQuery()
    {
        ...
        $q = $this->manifestationsService->completeQueryWithContact($q, $this->oauth->getToken()->OcTransaction->oc_professional_id ? $this->oauth->getToken()->OcTransaction->OcProfessional->Professional->contact_id : NULL
        );
    ...
    protected function postFormatEntity(array $entity, Doctrine_Record $manif, $options = [])
    {
        ...
        foreach ( $entity['gauges'] as $id => $gauge ) {
            ...
            if ( isset($gauge['availableUnits']) ) {
                ...
                $entity['gauges'][$id]['availableUnits'] = $free > $this->getMaxShownAvailableUnits() ? $this->getMaxShownAvailableUnits() : $free;
            }
        ...
        if ( !$includeEvents ) {
            //remove all entries starting with 'event'
            $entity = array_filter($entity, function($key) {
                return strpos($key, 'event') !== 0;
