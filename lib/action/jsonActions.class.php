...
abstract class jsonActions extends sfActions
{
    ...
    protected function createJsonResponse($data, $status = ApiHttpStatus::SUCCESS)
    {
        ...
        if ( !is_array($data) && !$data instanceof ArrayAccess ) {
            throw new ocException('Argument 1 passed to jsonActions::createJsonResponse() must implement interface ArrayAccess or be an array, ' . (is_object($data) ? get_class($data) : gettype($data)) . ' given.');
        }
