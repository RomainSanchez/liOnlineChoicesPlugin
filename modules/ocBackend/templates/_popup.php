<div style="position: fixed; top: 50%; left: 50%; width:50%; max-width: 600px; min-width: 500px; height: auto; transform: translateX(-50%) translateY(-50%); display:none;" 
  ...
  <div class="ui-dialog-content ui-widget-content" style="background-color: white;">
    ...
    <div class="sf_admin_actions_block floatleft">        
      <button data-url="<?php echo url_for('oc_backend/snapshot').'?type=save'; ?>" class="fg-button ui-widget ui-state-default ui-corner-all save_snapshot_popup"><?php echo __('Save', null, 'li_oc') ?></button>
    </div>
    <?php endif ?>
    <?php if ( $type == "load" ): ?>
    <p>
      <ul class="list_snapshots">
        ...
        <li>
          <a href="<?php echo url_for('oc_backend/snapshot').'?type=load&id='.$snapshot->id; ?>" target="_blank" class="snapshot">
            <?php echo "Charger l'état ".$snapshot->name.' du '.format_date(strtotime($snapshot->created_at), 'EEEE d MMMM yyyy HH:mm').' par '.$snapshot->sfGuardUser->name ?>
          </a>
        </li>
        <?php endforeach ?>
      </ul>
